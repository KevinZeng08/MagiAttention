...
struct Softmax {
  ...
  __forceinline__ __device__ TensorT max_get_scale(Tensor0& acc_s) {
    ...
    Tensor scores = make_tensor(acc_s.data(), flash::convert_layout_acc_rowcol<SwapAB>(acc_s.layout()));
    // TODO: reduce in a warpgroup
...
#pragma unroll
  ...
  __forceinline__ __device__ void rescale_o(Tensor1& acc_o, TensorT const& scores_scale) {
    ...
    Tensor acc_o_rowcol = make_tensor(acc_o.data(), flash::convert_layout_acc_rowcol<SwapAB>(acc_o.layout()));
