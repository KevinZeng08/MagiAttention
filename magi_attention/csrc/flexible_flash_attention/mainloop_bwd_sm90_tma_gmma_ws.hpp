...
template <int Stages, int Stages_dO, int Stages_dS, class ClusterShape_, class TileShape_MNK_, 
        class Element_, class ElementAccum_, class ArchTag_, bool Has_softcap_, bool Deterministic,
...
struct CollectiveMainloopBwdSm90 {
    ...
    using SmemLayoutAtomdQaccumTMA = decltype(cutlass::gemm::collective::detail::ss_smem_selector<GMMA::Major::K, ElementAccum,
        decltype(cute::get<0>(TileShape_MNK{})), Int<CUTE_STATIC_V(cute::get<2>(TileShape_MNK{})) / AtomLayoutMdQ>>());
    using SmemLayoutdQaccumTMA =
        decltype(tile_to_shape(SmemLayoutAtomdQaccumTMA{}, TileShape_dQaccum{}));
    using SmemLayoutdQaccumtTMA =
        decltype(cute::composition(SmemLayoutdQaccumTMA{},
                                   make_layout(make_shape(get<1>(TileShape_dQaccum{}), get<0>(TileShape_dQaccum{})),
                                               make_stride(decltype(get<0>(TileShape_dQaccum{})){}, _1{}))));
    ...
    store_dq(Params const& params,
        ...
        for (; m_block < m_block_max; ++m_block) {
            ...
            if (lane_predicate) { tma_store_wait<0>(); }
